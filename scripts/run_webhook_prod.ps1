...
if (-not $env:QQ_WEBHOOK_DEBUG) { $env:QQ_WEBHOOK_DEBUG = "1" }

$stdout = Join-Path (Get-Location) "webhook_prod.log"
$stderr = Join-Path (Get-Location) "webhook_prod.err.log"
Remove-Item -Force -ErrorAction SilentlyContinue $stdout, $stderr

# Build first so we can run the binary directly and capture its logs.
& "C:\\Users\\judge\\.cargo\\bin\\cargo.exe" build --example webhook_prod | Out-Null

$exe = Join-Path (Get-Location) "target\\debug\\examples\\webhook_prod.exe"
